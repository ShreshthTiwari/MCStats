    ...
    if (row) {
          ...
          if (rawData) {
            if (rawData[0] === "ONLINE") {
              ...
              if (!onlineSince) {
                ...
                await runQuery(
                  `UPDATE GLOBAL SET online_since = "${onlineSince}" WHERE guild_id LIKE "${interaction.guild.id}"`
                );
              }
      ...
      if (status === "ONLINE") {
        if (playersGrowthPercent) {
          ...
          await runQuery(
            `UPDATE GLOBAL SET players_total = ${playersTotal}, players_online = ${playersOnline} WHERE guild_id LIKE "${interaction.guild.id}"`
          );
        }
        ...
        if (displayUptime) {
          ...
          embed.addField(
            "UPTIME",
            `\`\`\`fix\n${((100 - downtime / total).toFixed(3) + "").replace(
              ".000",
              ""
            )}%\n\`\`\``
          );
          ...
          await runQuery(
            `UPDATE GLOBAL SET total = ${total}, downtime = ${downtime} WHERE guild_id LIKE "${interaction.guild.id}"`
          );
        }
        ...
        if (onlineSince) {
          await runQuery(
            `UPDATE GLOBAL SET online_since = null WHERE guild_id LIKE "${interaction.guild.id}"`
          );
        }
      }
    } else {
      await embed
        .setDescription(
          `Error occured while fetching the database.\nPlease report it to \`ShreshthTiwari#6014\`.`
        )
        .setColor(embedConfig.errorColor);
    }
