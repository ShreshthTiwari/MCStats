...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand(subcommand =>
      subcommand.setName('java').setDescription("Ping a Minecraft Java server.")
    ...
    .addSubcommand(subcommand =>
      subcommand.setName('bedrock').setDescription("Ping a Minecraft Bedrock server.")
        .addStringOption(option => option.setName("ip").setDescription("Bedrock server IP.").setRequired(true))
        .addIntegerOption(option => option.setName("bedrock_port").setDescription("Bedrock server port."))
    )
    .addSubcommand(subcommand =>
      subcommand.setName('fivem').setDescription("Ping a FiveM server.")
        .addStringOption(option => option.setName("ip").setDescription("FiveM server IP.").setRequired(true))
        .addIntegerOption(option => option.setName("fivem_port").setDescription("FiveM server port."))
    )
    .addSubcommand(subcommand =>
      subcommand.setName('samp').setDescription("Ping a SA-MP server.")
        .addStringOption(option => option.setName("ip").setDescription("SA-MP server IP.").setRequired(true))
        .addIntegerOption(option => option.setName("samp_port").setDescription("SA-MP server port."))
    ),
  ...
  async execute(client, MessageEmbed, embed, config, embedConfig, Permissions, interaction, messageEmojisReplacer, tick, cross, errorLogger, logger){
    ...
    if(subCommand === "java"){
      ...
      if(javaPort < 1 || javaPort > 65535){
        embed.setDescription(`${cross} Port cannot be less than 1 or greater than 65535.`)
      ...
      let fivemPort = await interaction.options.getInteger("fivem_port") || 30120;
      ...
      let sampPort = await interaction.options.getInteger("samp_port") || 7777;
      ...
      await interaction.editReply({embeds: [embed]}).catch(async error => {
        await errorLogger(client, interaction, error, "src/commands/ping.js : 492");
      });
    ...
    await interaction.editReply({embeds: [embed]}).catch(async error => {
      await errorLogger(client, interaction, error, "src/commands/ping.js : 499");
    });
  },
}