...
module.exports = {
  ...
  async execute(client, MessageEmbed, embed, config, embedConfig, database, Permissions, interaction, messageEmojisReplacer, tick, cross, errorLogger, logger){
    ...
    await embed.setDescription(`[**__B0T INVITE LINK__**](${config.inviteLink})`)
      .setColor(embedConfig.defaultColor);
    
    await interaction.editReply({embeds: [embed], components: [inviteButton]}).catch(async error => {
      await errorLogger(client, interaction, error, "src/commands/invite.js : 26");
    });
  },
}