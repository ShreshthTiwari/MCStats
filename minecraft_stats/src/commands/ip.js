...
module.exports = {
  ...
  async execute(client, MessageEmbed, embed, config, embedConfig, database, Permissions, interaction, messageEmojisReplacer, tick, cross, errorLogger, logger){
    ...
    db.serialize(async () => {
      db.each(`SELECT ip, java_port, bedrock_port, hidden_ports FROM GLOBAL WHERE guild_id like "${interaction.guild.id}"`, async (err, row) => {
        let IP = row.ip;
        ...
        if(!IP){
          ...
          await interaction.editReply({embeds: [embed]}).catch(async error => {
            await errorLogger(client, interaction, error, "src/commands/ip.js : 34");
          });
        ...
        if(javaPort && bedrockPort){
          ...
          await interaction.editReply({embeds: [embed]}).catch(async error => {
            await errorLogger(client, interaction, error, "src/commands/ip.js : 57");
          });
        ...
        await interaction.editReply({embeds: [embed]}).catch(async error => {
          await errorLogger(client, interaction, error, "src/commands/ip.js : 79");
        });
      });
    });
  },
