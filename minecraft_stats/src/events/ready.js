...
module.exports = {
  name: 'ready',
  once: true,
  async execute(client, embed, MessageEmbed, config, embedConfig, Permissions, messageEmojisReplacer, errorLogger, logger) {
    ...
    async function updater(){
      const rows = await fetchData(`SELECT * FROM GLOBAL WHERE (server_status_channel IS NOT NULL AND ip IS NOT NULL AND (java_port IS NOT NULL OR bedrock_port IS NOT NULL OR fivem_port IS NOT NULL OR samp_port IS NOT NULL))`, "all");
          ...
          if(guild){
            ...
            if(serverStatusChannel){
              ...
              if(javaPort){
                ...
                try{
                  let rawData = await fivemFetcher(client, guild.id, IP, fivemPort);
                  ...
                  if(rawData[0] === "ONLINE"){
                    ...
                    if(!hiddenPorts){
                      statusEmbed[guild.id].addField(`${wifi} PORT`, `\`\`\`fix\n${fivemPort}\n\`\`\``);
                    }
              ...
              statusEmbed[guild.id].addField("UPDATING", `<t:${Math.round(new Date().getTime()/1000) + (interval * 60)}:R>`)
                .setTimestamp();
              ...
              if(serverStatusMessageID[guild.id]){
                await channel[guild.id].messages.fetch(serverStatusMessageID[guild.id]).then(async (msg) => {
                  ...
                  await channel[guild.id].send({embeds: [statusEmbed[guild.id]]}).then(async (msg) => {
                    ...
                    const botUpdatesChannel = await guild.channels.cache.get(botUpdatesChannelID);

                    if(botUpdatesChannel){
                      embed = new MessageEmbed()
                        .setDescription(`Please give me following permissions in ${channel[guild.id]}-
                        ${branch}• \`VIEW CHANNEL\`
                        ${branch}• \`SEND MESSAGES\`
                        ${branchEnd}• \`READ MESSAGE HISTORY\``)
                        .setColor(embedConfig.errorColor);

                      await botUpdatesChannel.send({embeds: [embed]}).catch(error => {});
                    }
