  ...
  try {
    ...
    rawData = await srv
      ...
      .then((result) => {
        return [
          "ONLINE",
          result.icon,
          result.vars.banner_connecting,
          result.vars.locale,
          result.vars.sv_maxClients,
          result.vars.sv_projectDesc,
          result.vars.sv_projectName,
          result.vars.tags,
        ];
      })
      .catch(() => {});
    ...
    if (rawData[0] === "ONLINE") {
      rawData[rawData.length] =
        (await srv.getPlayers().then((result) => {
          return result;
        })) || 0;
      ...
      if (rawData[rawData.length - 1] > 0) {
        let players =
          (await srv.getPlayersAll().then((result) => {
            return result;
          })) || [];
